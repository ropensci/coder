...
Benchmarking will be performed by calculating Elixhauser comorbidity classes based on ICD-10 codes by corresponding functions from the `icd`, `icdswe` and `comorbidities.icd10`
packages.
...
```r
si <- devtools::session_info(c("icd", "icdswe", "comorbidities.icd10"))
si$packages <- 
  si$packages %>% 
  filter(package %in% c("icd", "icdswe", "comorbidities.icd10")) %>% 
  select(-source)
si
```
...
## 1 comorbidities.icd10 *   0.6.1 2016-04-13
## 2                 icd *   2.0.1 2016-04-27
## 3              icdswe *   0.0.1 2016-04-27
```
...
```r
uneval_list <- function(pkgs = c("icdswe", "icd", "comorbidities.icd10")) {
  
  # Initiate a list of expressions to benchmark
  expr <- list()
  
  # Include function "classify" from package "icdswe"
  if ("icdswe" %in% pkgs) expr$icdswe <- substitute(
...
```r
bm <- function(pkgs = c("icdswe", "icd", "comorbidities.icd10"), ...) {
...
```r
head(x <- dplyr::sample_n(icdswe::ex_icd10, 1e4, TRUE))
```
...
##                  id       date   code
##              <fctr>     <date> <fctr>
## 1      New, Brandon 2015-06-26   O892
## 2      Wong, Jaylin 2015-06-29  M069F
## 3   Bible, Cheyenne 2015-03-21   S599
## 4     Wright, Edwin 2015-07-08   P389
## 5 Ruiz-Smith, Angie 2015-10-26   S501
## 6      Ruder, Emily 2015-05-28   T923
```
...
A data set of 9910 is now remaining.
...
|:-------------------|-----:|-----:|-----:|------:|-----:|-----:|-----:|:---|
|icdswe              |  0.09|  0.09|  0.23|   0.09|  0.10|  0.79|     5|a   |
|icd                 |  6.33|  6.33|  6.36|   6.34|  6.35|  6.44|     5|b   |
|comorbidities.icd10 | 44.91| 45.04| 45.13|  45.06| 45.12| 45.51|     5|c   |
...
|expr                |    min|     lq|   mean| median|     uq|    max| neval|cld |
|:-------------------|------:|------:|------:|------:|------:|------:|-----:|:---|
|icdswe              |   1.00|   1.00|   1.00|   1.00|   1.00|   1.00|     5|a   |
|icd                 |  67.06|  67.59|  67.00|  67.12|  66.92|  66.32|     5|b   |
|comorbidities.icd10 | 480.24| 480.06| 477.44| 475.96| 474.76| 476.30|     5|c   |

We thus conclude that `icdswe` is faster than its competitors!
...
The `icd` package did perform better with RRO while `icdswe` and `comorbidities.icd10` were performing worse. The ranking among the packages were the same as above although the magnitude of the relative differneces were decreased between `icdswe` and `icd`.
...
`icd` and `icdswe` gives this "table" as a logical (boolean) matrix with patient id as row names. 
...
all_output <- function(...) {
  ...
  if (!is.null(res$icdswe)) 
    res$icdswe <-  res$icdswe[order(rownames(res$icdswe)), ]
...
|icdswe                          |icd          |comorbidities.icd10 |
|:-------------------------------|:------------|:-------------------|
...
```r
different <- res$icdswe != res$icd | res$icd != res$comorbidities.icd10
```

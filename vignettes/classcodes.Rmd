...
Here, `charlson` (as supplied by the `cc` argument) is a "classcodes" object containing a classification scheme matching patient codes (from `codedata`) to groups (comorbidity conditions) by regular expressions.

# Default classcodes

There are `r nrow(all_classcodes())` default "classcodes" objects in the package (`classcodes` column below). Each of them might have several versions of regular expressions (`regex`) and weighted indices (`indices`):
...
Each of those classcodes are documented (see for example `?charlson`). The object is basically a `tibble` (data frame) with some additional attributes (and a print method to present those):

```{r}
charlson
```

Columns have pre-specified names and content:
...
-   numeric variables used as weights when calculating index sums based on all (or a subset of) individual groups. (Column names are arbitrary but identified by `attr(., "indices")` .)
-   `condition`: (optional; not used with `charlson`) conditional classification (see for example `?hip_ae`).

In the example above, we did not specify which version of regular expression to use. We see from the printed output above (or by `attr(charlson, "regexprs")`), however, that the first listed regular expression (which is used by default) is "icd10". We have ICD-10 codes recorded in our code data set (`ex_icd10`). We could therefore use the alternative "icd10\_rcs" as well. Other versions might be relevant if the medical data is coded by other classifications (such as earlier versions of ICD). We will show below how to alter this setting in practice.
...
```{r}
url <- visualize(charlson, show = FALSE, regex = "icd10")
```

...
Let's consider the hypothetical example abow with some more concrete example data below. 
...
ex_atc <- 
  data.frame(
    ...
    id        = sample(ex_people$name[1:90], N, replace = TRUE),
...
Let's use an alternative version adopted from [Caughey](https://dx.doi.org/10.1016/j.diabres.2009.10.019) as specified by an argument passed by the `cc_args` argument. 
...
```{r}
# Import the magritr pipe for convenience
`%>%` <- magrittr::`%>%`
